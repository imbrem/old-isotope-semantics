...
Static Single Assignment, or SSA, is a cornerstone of modern compiler design, on which projects such as the LLVM compiler infrastructure \cite{llvm}, GCC's GIMPLE IR, Cranelift \cite{cranelift}, and MLIR \cite{mlir} are based. One of the core advantages of putting programs in SSA form is the ability to reason about programs in a more equational manner: since variables always have exactly one definition, we can justify complex optimizations like global value numbering (which would otherwise need to take into account complex flow-sensitive definitions) and more easily implement various analyses.

One of the interesting observations of the MLIR project is that SSA is very widely applicable, being useful for reasoning about everything from high-level machine learning operations to low-level GPU code, and even hardware design and simulation, as in the CIRCT project \cite{circt}. This hints that SSA is a natural syntax for some fundamental, hopefully compositional, structure, much like how the simply typed lambda calculus is a natural syntax for arbitrary Cartesian closed categories. In this talk, we would like to argue that SSA without control-flow is a natural syntax for \textit{Freyd categories}, which we then provide a graphical syntax for in terms of string diagrams as in \cite{promonad}. We then argue that SSA with \textit{terminating} control-flow (i.e., in which the basic blocks form a DAG) corresponds exactly to \textit{distributive} Freyd categories, while allowing general control-flow corresponds to equipping a distributive Freyd category with an \textit{Elgot structure}; we then give a string-diagrammatic graphical representation of such programs similar to traditional control-flow graphs. 

We do all this by giving an inductive, type-theoretic presentation of SSA which we equip with a categorical semantics, proving substitution and the soundness of various basic optimizations and rewrites. Finally, we wish to give some examples of various models and their applications, highlighting how they appear in diverse areas of computer science including classical compiler development, weak memory, quantum circuits, and hardware design.

Time permitting, we also hope to explain how to generalize our IR with support for both substructural types and impure substitutions, which require us to move to the more general setting of \textit{distributive effectful categories}, and give a few applications of this more general system in separation logic and reasoning about nondeterminism.
