...
since its introduction by \citet{ssa-original,rosen-gvn-1988} in the late 1980s.
Older compiler intermediate representations permitted the same local variable to
be assigned multiple times, whereas SSA requires each variable binding to be
...
tail-calls/jumps omit their arguments. Thus, the main difference between SSA and
a functional program is where in the program text the arguments to a tail call
are listed.
...
then rewrite SSA programs using the equational theory of functional programs.
...
finer classification of programs than merely value and expression to judge when
...
\begin{equation*}
\begin{BVerbatim}[baseline=c,fontsize=\small]
let x = print("hello");
let y = print("world");
let z = x;   
...
...
\end{BVerbatim}
\end{equation*}
In this case, substituting a term for a variable changes the order of I/O
actions, which changes the program's observable behaviour. On the other hand,
suppose we have an effect which increments a counter. Then in the following
program the same transformation is sound, since \(x\) is used exactly once:
\begin{equation*}
\begin{BVerbatim}[baseline=c,fontsize=\small]
let x = increment(a);
let y = increment(b);
...
can change this from a program which always returns \texttt{true} to one which
can return either \texttt{true} or \texttt{false}. On the other hand, if the
effect is the possibility of nontermination, then the substitution is sound.
\begin{equation}
\begin{BVerbatim}[baseline=c,fontsize=\small]
let x = may_loop();
let y = x;
...
in place, we can \emph{define} a pure term as one which is all three of central,
...
values. Having aliasing information justifies important optimisations such as
loop hoisting: 
\begin{equation}
\begin{BVerbatim}[baseline=c,fontsize=\small]
while (e) {
...
for compiler IRs, which can permit a separation of concerns. Compiler writers
can validate their optimisations against the abstract semantics, and we can
separately show that a particular machine model faithfully implements the
semantics.
...
substitution theorem and equational theory for both expressions and control-flow
graphs. This language includes substructural types, both for controlling effects
and tracking ownership in values. 
\item We give an axiomatic semantics for our language, by showing that it has a
compositional intepretation in any distributive effectful category (a
substructural variant of a Freyd category) with an Elgot structure. We use this
structure to give generic proofs of the soundness of substitution and rewriting
properties.
\item We show how our syntax quotiented by our equational theory is an initial model
of our semantics.
...
models satisfying these categorical axioms. Starting first from simple languages
...
decomposing it into basic blocks, which can be viewed as \textit{labels},
...
Figure \ref{fig:our-syn}. This does not introduce any new expressivity: an
...
matter. The quantity \(q\) indicates whether it is safe to duplicate an
instruction, or to delete it an if its result is unused. The instruction is
\textit{affine} if \(\ms{a} \in q\), \textit{relevant}, and \(\ms{r} \in q\),
and, if both affine and relevant, \textit{intuitionistic}. An instruction which
is, central, relevant, and affine is called \textit{pure}. We assume that \(p
\leq p'\) and \(q \subseteq r\) then \(\mc{I}_p^q(A, B) \supseteq
\mc{I}_{p'}^{r}(A, B)\). The set of types is generated by base types \(X\), the
\textit{unit type} \(\mb{1}\), the \textit{boolean type} \(\mb{2}\), and tensor
products of types \(A \otimes B\). We write \(\otimes\) (rather than \(\times\))
to underscore the fact that types are allowed to be substructural. For each base
type \(X\), if we are given a linearity \(\ms{lin}(X) \subseteq \tint\), we can
lift this to all types by taking \(\ms{lin}(A \otimes B) = \ms{lin}(A) \cap
\ms{lin}(B)\) and \(\ms{lin}(\mb{1}) = \ms{lin}(\mb{2}) = \tint\). We say a type
with \(\ms{a} \in \ms{lin}(A)\) is \textit{affine}, while a type with \(\ms{r}
\in \ms{lin}(A)\) is \textit{relevant}; a type which is both affine and relevant
is called \textit{intuitionistic}. Our syntax is composed of three main
...
\begin{itemize}
  ...
  \(A\), control how many times that variable can occur: if \(\ms{a} \in q \cap
  \ms{lin}(A)\) then the variable is called \textit{affine} or
  \textit{discardable} and it is permissible to leave it unused, whereas if
  \(\ms{r} \in q \cap \ms{lin}(A)\) it is called \textit{relevant} or
  \textit{duplicable} and it is permissible to use multiple times. As for
  instructions, a term which is central, relevant, and affine is called
  \textit{pure}. The rules for this judgement are given in Figure
  \ref{fig:ssa-term-typing}.
  
  \item \textit{Bodies} \(b\), which are typed via the judgement
  \(\hasty{\Gamma}{p}{b}{\Delta}{}\), which says that, given variables
  \(\Gamma\) live on entry, the variables \(\Delta\) are live on exit. The rules
  for this judgement are given in Figure \ref{fig:ssa-block-typing}.
  
  \item \textit{Terminators} \(s, t\), which are typed via the judgement
  \(\haslb{\Gamma}{t}{\ms{L}}\), which says that the terminator \(t\) is
  well-formed with respect to the context \(\Gamma\) and only branches to
  ("\textit{targets}") labels in \(\ms{L}\). The rules for this judgement are
  given in Figure \ref{fig:ssa-block-typing}.

  \item \textit{Basic blocks} \(\beta\), which are typed via the judgement
  \(\haslb{\Gamma}{\beta}{\ms{L}}\), which says that, given variables \(\Gamma\)
  live on entry, \(\beta\) only branches to labels in \(\ms{L}\). The rules for
  this judgement are given in Figure \ref{fig:ssa-block-typing}.

  \item \textit{Control-flow graphs} or \textit{CFG}s \(L\), which are typed via
    the judgement \(\lhaslb{\ms{L}}{L}{\ms{K}}\), which says that \(L\) defines
    a control-flow graph with input labels \(\ms{L}\) and output labels
    \(\ms{K}\); recursive control flow is allowed. The rules for this judgement
    are given in Figure \ref{fig:ssa-block-typing}.

  \item \textit{Regions}, which are a control-flow graph with a single
  distinguished entry block \(\beta\), and are typed with the judgement
  \(\haslb{\Gamma}{R}{\ms{L}}\), which says that, given variables \(\Gamma\)
  live on entry (which can be viewed as parameters to the region), the region
  has outputs \(\ms{L}\).
\end{itemize}
The formal grammar for our syntax is given in Figure \ref{fig:ssa-grammar} in
the appendix, while the typing judgements themselves are given in Figure
...
\(\ms{lin}(\thyp{x}{A}{q})\) is defined to be \(\ms{lin}(A) \cap q\). We say a
variable \(x: A^q\) is \textit{affine} if \(\ms{a} \in
\ms{lin}(\thyp{x}{A}{q})\), in which case it can be freely ignored. Similarly,
\(x\) is \textit{relevant} if \(\ms{r} \in \ms{lin}(\thyp{x}{A}{q})\), in which
case it can be used multiple times. This is formalized in the structural
judgement \(\csplits{\Gamma}{\Delta}{\Xi}\), which says that \(\Gamma\) can be
split into subcontexts \(\Delta\) and \(\Xi\) (weakening is simply the special
case \(\cwk{\Gamma}{\Delta} \iff \csplits{\Gamma}{\Delta}{\cdot}\)), as per the
...
to the left or right subcontext respectively; both these rules also allow us to
...
\(\ms{L}\) is a subcontext of \(\ms{K}\) and \(\Gamma\) is a weakening of
\(\Delta\), then \(\lwk{\ms{L}, \llhyp{\ell}{\Gamma}{A}}{\ms{K},
\llhyp{\ell}{\Delta}{A}}\), allowing us to "forget" any extra (affine) variables
dropped by \(\cwk{\Gamma}{\Delta}\). Finally, the rule \rle{join-zero} simply
says that if \(\lwk{\ms{L}}{\ms{K}}\), then \(\ms{L}\) is also a subcontext of
any extension of \(\ms{K}\).
...
\(b\), both as central terms. The quantity of a pair is simply the intersection
of the quantity of its components (expressed by allowing any quantity which
types \textit{both} components). Finally, \rle{let} and \rle{let2}, allow us to
type (destructuring) let-bindings. In both cases, we split the context
\(\Gamma\) into subcontexts \(\Delta, \Xi\), then, \(\Xi\) is used to type a
central term \(a\), which is bound to fresh variables appearing in a potentially
non-central term \(e\) which may also depend on variables in \(\Delta\).
...
Blocks do not have a type; instead, we judge which label-set \(\ms{L}\) a block
can branch to given that \(\Gamma\) is live on entry. In particular, \rle{br}
says that if \(\lwk{\lhyp{\ell}{\Delta}{A}}{\ms{L}}\), \(\Gamma\) splits into
\(\Delta\) and \(\Xi\), and \(a\) is a pure term which typechecks in \(\Xi\),
then \(\lbrb{\ell}{a}\) targets \(\ms{L}\) given \(\Gamma\) is live on entry.
\rle{ite} states that, if \(\Gamma\) splits into \(\Delta\) and \(\Xi\), \(e\)
typechecks as a pure, boolean term in \(\Xi\), and that, given all variables in
\(\Delta\) are live on entry, \(s, t\) both target \(\ms{L}\), then the
if-statement \(\ite{e}{s}{t}\) targets \(\ms{L}\) given \(\Gamma\) is live on
entry. \rle{let-blk} and \rle{let2-blk} are quite similar to their term
...
\begin{figure}
  ...
  \end{gather*}
  \caption{Typing rules for \isotopessa terminators, blocks, and regions}
...
begin with weakening, which states that we can add unused \textit{affine}
variables to a context, \textit{reduce} the quantity of an term's type, add
arbitrary unused labels, and remove \textit{affine} variables from existing
labels and still keep our typing judgements valid, i.e.
...
\begin{figure}
  ...
  \caption{ Typing rules for \isotopessa substitutions. The linearity of a
    context is defined recursively to be the intersection of the linearities of
    its variables: \(\ms{lin}(\cdot) = \tint\), \(\ms{lin}(\Gamma,
    \thyp{x}{A}{q}) = \ms{lin}(\Gamma) \cap \ms{lin}(\thyp{x}{A}{q})\). We
    assume that a substitution \(\ms{L}\) satisfies, for fresh \(x\),
...
\(\thyp{x}{A}{q}\) in \(\Gamma\) to a \textit{central} term \(\gamma(x)\). Due
to linearity, the resources \(\Theta\) must be split into subcontexts
\(\Theta_x\) which allow us to interpret each \(\gamma(x)\) as a term of type
\(A\) with quantity \(q\). This is described formally via the typing rules
...
\subseteq \ms{lin}(\Theta_x)\) (to avoid necessating the duplication/dropping of
substructural variables), and \(\Theta\) splits into \(\Theta_\Gamma\) and
...
solve this problem, we need to bind the terms in \(\gamma\) to variables with
the correct names for the live variable sets of desired targets. To do this, we
introduce the concept of a \textit{label-substitution} \(\mc{L}\), which we
define as a function that takes labels \(\lbl{\ell}\) and variables \(x\) to
blocks \(t\) such that, for any fresh variable \(x\), \([a/x]\mc{L}(\lbl{\ell},
x) = \mc{L}(\lbl{\ell}, a)\). We introduce the judgement
\(\lbsubst{\mc{L}}{\ms{L}}{\ms{K}}\) to mean that for each
\(\lbl{\ell}[\Gamma](x: A) \in \ms{L}\), we have that \(\mc{L}(\lbl{\ell}, x)\)
targets \(\ms{K}\). Formally, \rle{lb-subst-nil} states that this is vacuously
true for \(\ms{L} = \cdot\), while \rle{lb-subst-cons} says that if
...
\textit{object} to be affine or relevant as well. With this aim in mind, we
introduce the notion of a \textit{symmetric premonoidal subcategory}, as
follows:
...
\end{definition} 
Note that the symmetric premonoidal subcategories, monoidal subcategories, and
central subcategories of a category \(\mc{C}\) are all closed under
intersections of hom-sets, forming a complete meet-semilattice.
%
...
Applying the FP-style state transformer to the power-set monad yields the
Kliesli category with morphisms of the form \(\mc{C}_0(A, B) = A \to S \to
\mc{P}(A \times S)\). Taking \(\mc{C}_1\) to be the original category
...
general,
\begin{proposition}
  If \(\mc{C}\) is a substructural monoidal category and \(\mb{T}\) is a strong
  monad on \(\mc{C}\) then we can make the Kleisli category \(\mc{C}_{\mb{T}}\)
  into a substructural effectful category \(\upg{\cdot}{}: \mc{C} \to
  \mc{C}_{\mb{T}}\) by taking \(\ms{Aff}(\mc{C}_{\mb{T}}) = \ms{Aff}(\mc{C})\),
  \(\ms{Rel}(\mc{C}_{\mb{T}}) = \ms{Rel}(\mc{C})\), and \(\mc{C}_{\mb{T}}^q(A,
  B) = \upg{\mc{C}^q(A, B)}{}\), where \(\upg{f}{} = f;\eta\), and defining
  drops \(\upg{\ms{drop}(A)}{}\) and splits \(\upg{\ms{split}(A)}{}\).
\end{proposition}
...
To support branching, we need a bit more structure:
\begin{definition}[Distributive (pre)monoidal category] 
  ...
  \textit{rig categories} by \citet{laplaza-distributivity}, but for our
  purposes we will only make use of the following:
  \begin{equation*}
...
application \rle{app}, we first take \(\dnt{\hasty{\Gamma}{1}{a}{A}{q}}\), which
...
We interpret \textit{blocks} \(\haslb{\Gamma}{t}{\ms{L}}\) as \textit{impure}
morphisms from \(\dnt{\Gamma}\) to \(\dnt{\ms{L}}\). \(\Gamma\) represents all
the variables live on entry to the block, while \(\ms{L}\) represents all valid
targets, each annotated with live variables and an argument type; our semantics
are given in Figure \ref{fig:ssa-block-semantics}. \rle{br} is interpreted by
first splitting the context via \(\dnt{\csplits{\Gamma}{\Delta}{\Xi}}\), and
then using \(\Xi\) to compute the target's argument. The result, along with the
unchanged live variables \(\Delta\), is then passed to
...
either an output \(\dnt{\ms{K}, \llhyp{\ell}{\Gamma}{A}}\) or a recursive input
\(\dnt{\ms{L}}\), which we pass through unchanged as a recursive input. In the
former case, we can interpret this as either a value of \(\dnt{\ms{K}}\), which
we pass through unchanged as output, or \(\dnt{\Gamma} \otimes \dnt{A}\); which
we can pass as input to \(\dnt{\haslb{\Gamma, \thyp{x}{A}{q}}{t}{\ms{L}}}\),
giving us a value of \(\ms{L}\) we can use as recursive input. Note that, even
if \(t\) yields an output label in \(\ms{K}\), we still return a
\textit{recursive input}, not an output; in this case, the \textit{next}
iteration will return it as an output. We can now quite easily state the
semantics of \rle{where}: we simply precompose the semantics of the entry block
\(\dnt{\haslb{\Gamma}{t}{\ms{L}}}\) with the \textit{iterated} semantics of the
associated control-flow graph, \(\dnt{\lhaslb{\ms{L}}{L}{\ms{K}}}^\dagger\).
...
\(\gamma(x)\). Similarly, we interpret label-substitutions as morphisms from
\(\dnt{\ms{L}}\) to \(\dnt{\ms{K}}\) which, for each input label in \(\ms{L}\),
performs a computation targeting \(\ms{K}\). In particular, \rle{lb-subst-nil}
is interpreted as the zero morphism \(0_{\dnt{\ms{K}}}:
...
\begin{theorem}[Semantic Substitution] 
  ...
  \begin{itemize}
    ...
    \[
      ...
      ; \dnt{\haslb{\Gamma}{t}{\ms{L}}}
      = \dnt{\haslb{\Theta}{\exprletsubst{\substctx{\gamma}{\Gamma}}{t}}{\ms{K}}} 
...
properties of model-dependent operations (such as memory operations). Because we
have a compositional denotational semantics, equations automatically lift into a
full congruence. This lets us justify optimisations such as scalar replacement
of aggregates, eliminating redundant writes, reordering independent memory
operations, and memory-to-register transformations.
...
In this section, we give cncrete models of the axiomatisation given in Section
\ref{sec:semantics} to with successively more sophisticated features such as
weak memory and separation logic.
...
\begin{definition}[Submonad] 
  ...
  \mb{S}\;A\), \(f: A \to \mb{S}\;B\), \(\ms{bind}\;s\;f \in \mb{S}\;A\). We say
...
\end{itemize}
We can clearly see that this is a refinement type system in the sense of
\citet{mellies-ftrs}: in particular, there is a forgetful functor \((A, \Phi)
...
\begin{lemma} We have that
  \begin{enumerate}
    \item Splitting commutes: if \(\csplits{\Gamma}{\Delta}{\Xi}\) then \(\csplits{\Gamma}{\Xi}{\Delta}\), and \(\dnt{\csplits{\Gamma}{\Delta}{\Xi}} = \dnt{\csplits{\Gamma}{\Xi}{\Delta}};\sigma\)
    \item Splitting associates: if \(\csplits{\Gamma}{\Delta}{\Xi}\) and \(\csplits{\Xi}{\Theta}{\Phi}\) then there exists \(\Gamma_{\Delta, \Theta}\) such that \(\csplits{\Gamma}{\Gamma_{\Delta, \Theta}}{\Phi}\) and \(\csplits{\Gamma_{\Delta, \Theta}}{\Delta}{\Theta}\), and, for \emph{all} such \(\Gamma_{\Delta, \Theta}\), we have \(\dnt{\csplits{\Gamma}{\Delta}{\Xi}};\dnt{\Delta} \otimes \csplits{\Xi}{\Theta}{\Phi} = \dnt{\csplits{\Gamma}{\Gamma_{\Delta, \Theta}}{\Phi}};\dnt{\csplits{\Gamma_{\Delta, \Theta}}{\Delta}{\Theta}} \otimes \dnt{\Phi};\alpha\)
    \item Weakening composes: if \(\cwk{\Gamma}{\Delta}\) and \(\cwk{\Delta}{\Xi}\), then \(\cwk{\Gamma}{\Xi}\)
    \item Splitting and weakening compose on the right: if \(\csplits{\Gamma}{\Delta}{\Xi}\) and \(\cwk{\Delta}{\Delta'}\) then \(\csplits{\Gamma}{\Delta'}{\Xi}\), furthermore,
    \(\dnt{\csplits{\Gamma}{\Delta'}{\Xi}} = \dnt{\csplits{\Gamma}{\Delta}{\Xi}};\dnt{\cwk{\Delta}{\Delta'}} \otimes \dnt{\Xi}\)
    \item Splitting and weakening compose on the left: if \(\csplits{\Gamma}{\Delta}{\Xi}\) and \(\cwk{\Gamma'}{\Gamma}\) then \(\csplits{\Gamma'}{\Delta'}{\Xi}\), furthermore,
    \(\dnt{\csplits{\Gamma'}{\Delta}{\Xi}} = \dnt{\cwk{\Gamma'}{\Gamma}};\dnt{\csplits{\Gamma}{\Delta}{\Xi}}\)
  \end{enumerate}
\end{lemma}
...
\begin{lemma}[Substitution Splitting] 
  ...
  \(\csplits{\Theta}{\Theta_\Delta}{\Theta_\Xi}\),
  \(\issubst{\gamma}{\Theta_\Delta}{\Delta}\), and
  \(\issubst{\gamma}{\Theta_\Xi}{\Xi}\). In particular, if
  \(\cwk{\Gamma}{\Delta}\) and \(\issubst{\gamma}{\Theta}{\Gamma}\), then
  \(\issubst{\gamma}{\Theta}{\Delta}\).
\end{lemma}
...
\begin{lemma}[Substitution] 
  Given \(\issubst{\gamma}{\Theta}{\Gamma}\),
  \begin{enumerate}
    \item If \(\hasty{\Gamma}{p}{a}{A}{q}\), then \(\hasty{\Theta}{p}{[\gamma]a}{A}{q}\) \label{lemma:substitution:term}
    ...
    \label{lemma:substitution:block}
    \item If \(\lhaslb{\ms{W}}{L}{\ms{L}}\), then there exists \(\ms{K},
    \ms{W}'\) such that \(\lbsubst{\substlbs{\gamma}{\ms{L}}}{\ms{K}}{\ms{L}}\),
    \(\lbsubst{\substlbs{\gamma}{\ms{W}'}}{\ms{W}}{\ms{W}'}\), and
    \(\lhaslb{\ms{W}'}{[\gamma]L}{\ms{K}}\)
    \label{lemma:substitution:cfg}
  \end{enumerate}
\end{lemma}

\begin{proof} 
  We begin by proving \ref{lemma:substitution:term} by induction on derivations \(\hasty{\Gamma}{p}{a}{A}{q}\):
  \begin{itemize}
    \item \rle{var}: we have that \(\cwk{\Gamma}{\thyp{x}{A}{q}}\), and
    therefore that \(\issubst{\gamma}{\Theta}{\thyp{x}{A}{q}}\); by
    \rle{subst-cons}, we obtain the desired result that
    \(\hasty{\Theta}{p}{\gamma(x)}{A}{q}\).
    \item \rle{app}: by induction, we have that
    \(\hasty{\Theta}{1}{[\gamma]a}{A}{q}\), and therefore we can apply \rle{app}
    to obtain \(\hasty{\Theta}{p}{[\gamma](f\;a)}{B}{q}\) as desired.
    \item \rle{pair}: we have that \(\issubst{\gamma}{\Theta_\Delta}{\Delta}\) and
    \(\issubst{\gamma}{\Theta_\Xi}{\Xi}\). Therefore, by induction, we have that
    \(\hasty{\Theta_\Delta}{1}{[\gamma]a}{A}{q}\) and
    \(\hasty{\Theta_\Xi}{1}{[\gamma]b}{B}{q}\); we can simply apply \rle{pair}
    to obtain our desired result that \(\hasty{\Theta}{p}{[\gamma](a, b)}{A
    \otimes B}{q}\)
    \item \rle{true}, \rle{unit}, \rle{false}: these cases hold trivially since
    \(\cwk{\Theta}{\cdot}\)
    \item \rle{let}: we have that \(\issubst{\gamma}{\Theta_\Delta}{\Delta}\)
    and \(\issubst{\gamma}{\Theta_\Xi}{\Xi}\). Taking \(x\) to be a fresh
    variable (with \(\gamma(x) = x\)), we can conclude by \rle{subst-cons} that
    \(\issubst{\gamma}{\Theta_\Delta, \thyp{x}{A}{}}{\Delta, \thyp{x}{A}{}}\).
    Therefore, by induction, we have that \(\hasty{\Theta_\Delta, \thyp{x}{A}{}}{p}{[\gamma]e}{B}{q}\) and \(\hasty{\Theta_\Xi}{1}{[\gamma]a}{A}{q}\); we
    can hence simply apply \rle{let} to obtain
    \(\hasty{\Theta}{p}{[\gamma]\letexpr{x}{a}{e}}{B}{q}\), as desired.
    \item \rle{let2}: we have that \(\issubst{\gamma}{\Theta_\Delta}{\Delta}\)
    and \(\issubst{\gamma}{\Theta_\Xi}{\Xi}\). Taking \(x, y\) to be fresh
    variables (with \(\gamma(x) = x\), \(\gamma(y) = y\)), we can conclude by
    \rle{subst-cons} that \(\issubst{\gamma}{\Theta_\Delta, \thyp{x}{A}{},
    \thyp{y}{B}{}}{\Delta, \thyp{x}{A}{}, \thyp{y}{B}{}}\). Therefore, by
    induction, we have that \(\hasty{\Theta_\Delta, \thyp{x}{A}{},
    \thyp{y}{B}{}}{p}{[\gamma]e}{C}{q}\) and
    \(\hasty{\Theta_\Xi}{1}{[\gamma]a}{A \otimes B}{q}\); we can hence simply
    apply \rle{let2} to obtain
    \(\hasty{\Theta}{p}{[\gamma]\letexpr{(x, y)}{a}{e}}{C}{q}\), as desired.
  \end{itemize}
  Using \ref{lemma:substitution:term}; we can now prove
  \ref{lemma:substitution:block} and \ref{lemma:substitution:cfg} by mutual
  induction on derivations as follows:
  \begin{itemize}
    \item \rle{br}: \todo{this}
    \item \rle{let-blk}: \todo{this}
    \item \rle{ite}: \todo{this}
    \item \rle{let2-blk}: \todo{this}
    \item \rle{where}: \todo{this}
    \item \rle{nil-br}: \todo{this}
    \item \rle{cons-br}: \todo{this}
  \end{itemize}
\end{proof}
...
\begin{proof} 
  We proceed by mutual induction on derivations as follows:
  \begin{itemize}
    \item \rle{br}: \todo{this}
    \item \rle{let-blk}: \todo{this}
    \item \rle{ite}: \todo{this}
...
\begin{lemma}[Semantic Upgrade]
  If \(\hasty{\Gamma}{1}{a}{A}{q}\), we have that
  \(\dnt{\hasty{\Gamma}{0}{a}{A}{q}} =
  \upg{\dnt{\hasty{\Gamma}{1}{a}{A}{q}}}{}\)
\end{lemma}
...
\begin{figure}
  \begin{grammar}
    ...
    <\(M\)> ::= \(\tyhole{\lhole{X}}{\Gamma}{p}{A}{q}\)
    \;|\; \(\blkhole{\lhole{T}}{\Gamma}{\ms{L}}\)
    \;|\; \(\cfghole{\lhole{W}}{\ms{L}}{\ms{K}}\)
  \end{grammar}
...
\end{figure}
We introduce a typing judgement \(\isrw{\mc{H}}{I}{H}\), which says that
\(\mc{H}\) assigns to each hole in \(H\) a term satisfying the corresponding
typing judgement within the hole context \(I\). We also introduce new typing
judgements for terms, blocks, and CFGs with holes in Figure
\ref{fig:ssa-holes-judgements}. In each case, they correspond to the judgement
that, given a collection \(\mc{H}\) terms, blocks, and CFGs to fill each hole in
\(H\), would satisfy the corresponding judgement without holes in
\ref{fig:ssa-judgements}.
...
\begin{figure}
  \begin{gather*}    
    \prftree[r]{\rle{hole-tm}}
      {\tyhole{\lhole{X}}{\Delta}{p}{A}{q} \in H}
      {\cwk{\Gamma}{\Delta}}
...
\begin{figure}
  \begin{gather*}
    \dnt{\tyhole{\lhole{X}}{\Gamma}{p}{A}{q}} 
      = \mc{C}_p^q(\dnt{\Gamma}, \dnt{A})
    \qquad
    \dnt{\blkhole{\lhole{T}}{\Gamma}{\ms{L}}} 
      = \mc{C}_0(\dnt{\Gamma}, \dnt{\ms{L}})
      \\
    \dnt{\cfghole{\lhole{W}}{\ms{L}}{\ms{K}}}
      = \mc{C}_0(\dnt{\ms{L}}, \dnt{\ms{K}} + \dnt{\ms{L}}) 
      \\
    \dnt{H, M} = \dnt{H} \times \dnt{M}
    \qquad
    \dnt{\cdot} = \mb{1}
      \\
    \boxed{\dnt{M \in H}: \dnt{H} \to \dnt{M}}
      \qquad
    \dnt{M \in H, M \in H} 
      = \pi_1
    \qquad
    \dnt{M \in H, N} = \pi_0;\dnt{M \in H}
  \end{gather*}
...
\begin{figure}
  \begin{gather*}
    ...
    \sorry
      \\
    \boxed{
      \dnt{\tyhole{\mhole{X}}{\Gamma}{a}{A}{q} \in H}
      : H \to \mc{C}^q_p(\dnt{\Gamma}, \dnt{A})
    }
  \end{gather*}
...
\end{figure}
Of course, as for terms without holes, we have a coherence lemma:
\begin{lemma}[Rewriting Coherence] \
  ...
  \end{itemize}
\end{lemma}
We may finally state the semantic rewriting theorem as follows:
\begin{theorem}[Semantic Rewriting]
  ...
  \]
\end{theorem}
This justifies congruence based optimizations, and in particular E-graph
rewriting.
...
In general, we often want to justify optimizations which \textit{refine} a
program by, for example, reducing the set of allowable behaviours, rather than
simply rewrite the program to an equivalent one. One place where refinement
pops up quite often is in the treatment of undefined values and undefined 
behaviour; for example, in a language where reading a dangling pointer is UB, 
the transformation 
\begin{equation*}
\begin{BVerbatim}[baseline=c, fontsize=\small]
  let x = read(p);
...
\end{BVerbatim}
\end{equation*}
is valid, but the reverse is \textit{not}, since that would \textit{introduce},
rather than \textit{remove}, potential UB. These optimizations are particularly
important when operations are allowed to generate undefined values, in which
case, even simple rewrites such as \(a \times 0 \to 0\) and \(a - a \to 0\) are
refinements rather than equalities. In this appendix section, we will generalize
our semantics for rewriting from section \ref{apx:rewriting} to support
reasoning about refinements in a compositional manner.

\subsection{Poset-Enriched Categories}

To reason about refinement, we need to introduce the definition of a
\textit{poset-enriched category}:
\begin{definition}[Poset-Enriched Category]
  A category \(\mc{C}\) is \textbf{poset-enriched} if, for all objects \(A, B
  \in |\mc{C}|\), \(\mc{C}(A, B)\) is equipped with a partial order \(\leq\)
  compatible with composition, i.e. \(f \leq g \implies f;h \leq g;h, h';f \leq
  h';g\). A \textbf{poset-enriched premonoidal category} is a poset-enriched
  category such that the partial order on morphisms is compatible with
  whiskering, i.e. \(f \leq g \implies f \otimes C \leq g \otimes C, C \otimes f
  \leq C \otimes g\). A \textbf{poset-enriched substructural category} is a
  poset-enriched premonoidal category such that the subcategories
  \(\mc{C}^{\ms{a}}, \mc{C}^{\ms{r}}\) are downward-closed under \(\leq\). 
\end{definition}
Note that every (premonoidal/substructural) category \(\mc{C}\) can be
considered poset-enriched under the discrete partial order \((\leq) = (=)\). One
useful example of a poset-enriched substructural category is the category of
relations from section \ref{ssec:substruct}, taking \(f \leq g \iff \forall a
\in A, f(a) \subseteq g(a)\).

\subsection{Semantic Refinement}

\TODO{refinement for rewrites}

\TODO{semantic refinement theorem}
...
\subsection{Unrolling and Vectorization}
...
\subsection{Loop Fusion/Fission}

\TODO{this}

\section{E-Graph Optimizations}

\TODO{this}

\subsection{Floating SSA}

\TODO{this}

\subsection{Whole-program rewriting}

\TODO{this}

% \section{State-Passing Category}
%
