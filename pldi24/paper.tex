...
former, but this definition does \textit{not} support sliding:
\begin{equation}
...
model, we need to introduce a \textit{buffer} of type \(\ms{Buf} = \mc{A}^*\) of
lists of actions, which we will interpret as linear pomsets ordered by index. In particular, we define the monad \(\ms{TSO} = \ms{StateT}\;\ms{Buf}\;\ms{PO}\), which gives us another \isotopessa model in the usual manner.
