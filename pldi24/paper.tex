...
\begin{definition}[Submonad] 
  ...
  \begin{enumerate}
    ...
    submonads, forming a complete meet-semilattice with bottom \(\eta\) and top
    \(\mb{T}\),
...
notion of refinement types, while at the same time giving us a nontrivial notion
of nonlinear types. Taking as objects pairs \((A, \Phi)\), where \(\Phi: A \to
...
concurrent operations affecting our potential executions. On that note, we can
define the \textit{parallel execution} of two morphisms as follows:
\begin{equation}
  \begin{aligned}
