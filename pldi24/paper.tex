...
\begin{proof} 
  ...
  \begin{itemize}
    ...
    \rle{subst-var}, we obtain the desired result that
    \(\hasty{\Theta}{p}{\gamma(x)}{A}{q}\).
    \item \rle{app}: \todo{this}
    \item \rle{pair}: \todo{this}
    \item \rle{true}: \todo{this}
    \item \rle{false}: \todo{this}
    \item \rle{let}: \todo{this}
    \item \rle{unit}: \todo{this}
    \item \rle{let2}: \todo{this}
  \end{itemize}
  Using \ref{lemma:substitution:term}; we can now prove
  \ref{lemma:substitution:block} and \ref{lemma:substitution:cfg} by
  mutual induction on derivations as follows:
  \begin{itemize}
...
\begin{theorem}[Semantic Rewriting]
  ...
  \]
\end{theorem}
This justifies congruence based optimizations, and in particular E-graph rewriting.
