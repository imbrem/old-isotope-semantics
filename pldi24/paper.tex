...
\TODO{pull Elgot structure up here?}

\TODO{substructural premonoidal category, relationship? This makes \(\mc{C}_0^q\) have structure, which is useful at least for things that are affine but not central (see: dead code elimination). Duplication is Less Useful (TM) but nice for symmetry.}
...
\begin{definition}[Distributive category] 
  A \textbf{distributive category} is a premonoidal category with coproducts equipped with a natural family of central isomorphisms \(\delta_{A, B, C}: (A + B) \otimes C \simeq A \otimes C + B \otimes C\).
\end{definition}
...
Some important properties of all Elgot categories include:
\begin{itemize}
  \item \textbf{Dinaturality:} \((g;[\iota_0, h])^\dagger = g;[\ms{id}, (h;[\iota_0, g])^\dagger]\). \TODO{Proposition 16 of \cite{goncharov-guarded-unguarded}; also Lemma 31 of \cite{goncharov-squaring}}
  \item \textbf{Squaring:} \((f;[\iota_0, f])^\dagger = f^\dagger\). \TODO{Lemma 31 of \cite{goncharov-squaring}}
\end{itemize}
\TODO{Hyland and Hasegawa traces?}
\TODO{intuition? Can draw diagrams, but those take up space}

\TODO{results on distributivity of monads}

\TODO{result: if a monad is Elgot, the state/reader/writer transformers yield Elgot monads}

\TODO{example of Elgot monads: traces}

\subsection{Denotational Semantics}

\TODO{text}

The semantics for an \isotopessa dialect with a set of types \(\mc{T}\) and instructions \(\mc{I}\) are given by:
\begin{itemize}
  \item A \textit{substructural effectful category} \(\mc{C}\) such that \(\mc{C}_0\) is \textit{distributive} and \textit{Elgot}, equipped with
  \item A function \(\ms{base}(X)\) from base types \(X \in \mc{T}\) to objects in \(|\mc{C}|\), such that \(\ms{a} \in \ms{lin}(X) \implies X \in \ms{Aff}(\mc{C})\) and \(\ms{r} \in \ms{lin}(X) \implies X \in \ms{Rel}(\mc{C})\)
