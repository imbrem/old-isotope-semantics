...
equational theory for use in justifying optimisations. \todo{Their
...
We are the first paper to connect a weak memory semantics to the
semantics of SSA. In order to establish a rich equational theory,
we needed a number of features of the semantics which are sometimes
difficult to come by in weak memory semantics. First, sequencing
must be associative (i.e., $(t_1; t_2); t_3 \equiv t_1; (t_2; t_3)$).
Second, the semantics must support loops, including the equation
that a while-loop is equal to its unrolling. Finally, booleans need
to be distributive (i.e., $(A \otimes \mb{2}) \simeq (A + A)$). 
