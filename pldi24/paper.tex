...
model, we need to introduce a \textit{buffer} of type \(\ms{Buf} = \mc{A}_w^*\)
of lists of write actions, which we will interpret as linear pomsets ordered by
index. In particular, we define the monad \(\ms{TSO} =
\ms{StateT}\;\ms{Buf}\;\ms{PO}\). Given a buffer \(\ms{Buf}\), we can define the
result of reads \([\cdot]_x: \ms{Buf} \to \ms{Word} \sqcup \{\bot\}\) from the
buffer as follows:
\begin{equation}
  (L;\{x := v\})[x] = v
  \qquad
...
writes overwrite earlier ones! We will implement TSO ordering by buffering all
our writes, in which case they are only visible to the local thread. At any
point, we may choose to \textit{flush} some of the writes in buffer to the
global pomset of events. This is implemented via the morphism (called
\(\ms{split}\) in \cite{sparky})
\begin{equation}
  \ms{pflush} = \lambda ()\;L. \{((), L', L'') | L = L';L''\}
...
\begin{equation}
  ...
  \dnt{\ms{write}_x} = W_x^{\ms{TSO}}
    &= \ms{pflush};(\lambda v\;L. \{(v, L)\});\ms{pflush}
  \end{aligned}
\end{equation}
