...
of the writes in buffer to the global pomset of events.
\TODO{\(\mc{A}_{\ms{TSO}}\), \(\mc{A}_b\)}
of type \(\ms{Buf} = \mc{A}_b^*\)
...
\ms{StateT}\;\ms{Buf}\;(\ms{Trace}\;\Sigma)\). 
\TODO{using \(\mc{A}_{\ms{TSO}}\)}
Given a buffer \(\ms{Buf}\), we can define the
