...
\TODO{define a substructural effectful category, rewrite as ``substructural effectful category \(\mc{C}\)" equipped with proper structrure}
...
\begin{itemize}
  ...
  \item A mapping from base types \(A\) to objects in \(|\mc{C}|\)
  \item A mapping from functions \(f \in \mc{I}_p^q(A, B)\) to morphisms \(\dnt{f}: \mc{C}_p^q(A, B)\), where \(\mc{C}_0^q = \mc{C}_0\)
  \item 
\end{itemize}
