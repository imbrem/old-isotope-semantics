...
Now that we have a semantics for \isotopessa programs, we can show
that it matches the syntax by showing that the syntactic and semantic
notions of substitution coincide. 

\todo{Move weakening and upgrade lemmas to appendix}
% Since we're
% approaching things from a type-theoretic perspective, we begin by addressing the
% semantic versions of the standard syntactic metatheorems of weakening and
% substitution. 
% 
% In particular, semantic weakening can be stated in the expected
% manner: if we have an expression or block which is well typed in both \(\Gamma\)
% and \(\Delta\), where \(\cwk{\Gamma}{\Delta}\), then evaluating it in \(\Gamma\)
% is the same as first strengthening \(\Gamma\) to \(\Delta\), dropping unused
% affine variables, and then evaluating it in \(\Delta\).
...
\TODO{some text about substitution semantics -- substitution = composition of env semantics with therm semantics}
\TODO{some text about semantic substitution -- it's a function that does the obvious thing}
