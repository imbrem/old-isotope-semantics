...
\begin{itemize}
  ...
  for this judgement are given in Figure \ref{fig:ssa-term-typing}; in
  particular, we note that a impure terms cannot be \textit{nested}; that is,
  all proper subterms of any term are pure.
...
\TODO{segue to label-contexts \(\ms{L}\)}

The rule \rle{join-nil} is the analogue to \rle{split-nil} for \textit{label-contexts}, providing a base case in that the empty label weakens into itself. The rule \rle{join-cons} says that if \(\ms{L}\) weakens into \(\ms{K}\) and \(\Delta\) weakens into \(\Gamma\), then \(\lwk{\ms{L}, \llhyp{\ell}{\Gamma}{A}}{\ms{K}, \llhyp{\ell}{\Delta}{A}}\), allowing us to "forget" any extra variables dropped by \(\cwk{\Gamma}{\Delta}\). Finally, the rule \rle{join-zero} simply says that if \(\lwk{\ms{L}}{\ms{K}}\), then \(\ms{L}\) also weakens to any extension of \(\ms{K}\).

\TODO{label-weakening is backwards}
