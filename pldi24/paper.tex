...
\TODO{define premonoidal category?}
...
\begin{definition}[Stream Action]
  A \textbf{stream action} of a monoid \(M\) on a set \(I\) is a pair \((\Pi, \cdot)\) where \(\Pi: M^\omega \to I\) is a map from streams of \(M\) to \(I\) and \(\cdot: M \to I \to I\) is a monoid action such that
  \(
    m \cdot \Pi \ell = \Pi (m: \ell) 
  \)
  where \(m:\ell\) denotes the stream formed by prefixing \(m\) to \(\ell\).
\end{definition}
Two important examples include:
\begin{itemize}
  \item For any monoid \(M\), the unique maps \(\Pi: (\nats \to M) \to \mb{1}\), \(\cdot: M \to \mb{1} \to \mb{1}\) form a stream action on \(\mb{1}\)
  \item The concatenation map \(\Pi: (A^*)^\omega \to A^\omega\) and the concatenation action \(\cdot: A^* \to A^{\leq \omega} \to A^{\leq \omega}\) form a stream action of the monoid of lists of \(A\), \(A^*\), on the set of potentially infinite sequences of \(A\), \(A^{\leq \omega}\).
\end{itemize}

\TODO{text}

\begin{definition}[Trace Monad]
  Given a stream action \((\Pi, \cdot)\) of \(M\) on \(I\), we can define the \textbf{trace monad} \(\ms{Trace}\;\Pi\;A = A \times M + I\), with
  \(\eta_A = \lambda a.\iota_0 (a, 1)\)
  and \(\mu_A = [\lambda (a, m). m \cdot a, \ms{id}]\)
  where the action of \(M\) on traces is given by \(m \cdot \iota_0 (a, m') = \iota_0 (a, mm')\) and \(m \cdot \iota_1 t = \iota_1 (m \cdot t)\). This is an Elgot monad, with
  \[f^\dagger(a) = ...\]
\end{definition}

\TODO{text}

\begin{definition}[Nondeterministic Trace Monad]
  Given a stream action \((\Pi, \cdot)\) of \(M\) on \(I\), we can define the \textbf{nondeterministic trace monad} \(\ms{Trace}\;\Sigma\;A = \mc{P}^+(A \times M + I)\), with
  \(\eta_A = \lambda a.\{\iota_0 (a, 1)\}\)
  and \(\mu_A = ...\)
  This is an Elgot monad, with
  \[f^\dagger(a) = ...\]
\end{definition}

\TODO{text}

\TODO{induces a substructural effectful category...}

\TODO{central = commutative, "commutes with infinity" (i.e. is absorbed by infinity)}

\TODO{relevant = deterministic}

\TODO{affine = no effect on trace}
...
We'll start by considering the category \(\ms{HRel}\), which we will take to have morphisms of the form \(A \to \ms{Heap} \to \mc{P}^+(A \times \ms{Heap} \times \ms{Bytes}^* + \ms{Bytes}^{\leq \omega})\), which is the result of applying the state-transformer monad with state \(\ms{Heap}\) to the trace monad with the infinite monoid of lists of strings described in the previous section.

\TODO{Name for ND trace monad}

% \TODO{by the above, this is an \isotopessa model w/ nice properties; moving on!}

% We now define a separation logic with predicates having the following grammar:
% \begin{grammar}
%   <\(\varphi, \psi\)> ::= 
%     \(\varnothing\) 
%     \;|\; \(e \mapsto a\) 
...
%     \;|\; \(\bot\)
% \end{grammar}
% where \(e, a\) denote regular \isotopessa expressions and \(A, B\) regular \isotopessa types.

% We can now do something somewhat tricky with \isotopessa dialects:
% \TODO{truly pure expressions \(\implies\) semantics for expressions}
% \TODO{giving us a semantics for separation logic stuff}
% \TODO{now we define the category \(\ms{Sep}\) w/ objects contexts + predicates}
% \TODO{tensor product, niceties \(\implies\) category, yey}
% \TODO{sliding}
% \TODO{therefore, substructural \textit{effectful} w/ nontrivial linear objects}
