...
Our grammar proper splits program fragments into three main syntactic categories:
\begin{itemize}
  \item \textit{Expressions} \(a, b, c, d, e\), which are typed via the
  judgement \(\hasty{\Gamma}{p}{a}{A}{q}\), which says that the term \(a\)
  ...
  term for a variable, and \(q\), plus the linearity of \(A\), control how
  ...
  times. An term which is central, relevant, and affine is called
...
\(\Gamma\) are affine. Note that the quantity \(q\) of the \textit{term}
...
\(B\) and a \textit{central} term \(\hasty{\Gamma}{1}{a}{A}{q}\), we
...
us to perform relatively weak optimistions in the presence of effects. To
...
the identity object considered affine and relevant. Similarly, every
...
"substructural Cartesian category." We also note the following:
\begin{proposition}
...
\begin{definition}[Trace Monad]
  ...
  \iota_0 (a, mm')\) and \(m \cdot \iota_1 t = \iota_1 (m \cdot t)\). The iteration operator \(f^\dagger(a) = f^\infty(a) \cup
  \bigcup_{n \in \nats}\{\iota_0 (\iota_0\;b, m) \in f_n(a)\} \cup \{\iota_1 t
  \in f_n(a)\}\) yields an Elgot structure, where
  \[
...
\begin{figure}
  \begin{center}        
    ...
    \begin{tabular}{rl}
        \multicolumn{1}{c}{Judgment} & \multicolumn{1}{c}{Meaning} \\ \hline
        \(\isrw{\mc{H}}{I}{H}\) & \(\mc{H}\) fills every hole in \(H\), modulo
        holes \(I\) \\
...
\ref{fig:ssa-block-holes-typing}.
...
This formalism allows us to state the \textit{syntactic rewriting} theorem,
which says, in essence, that filling holes with valid terms yields valid terms.
