...
pass in as recursive input. Note that, even if \(t\) branches to an output label
in \(\ms{K}\), we still return a \textit{recursive input}, not an output; in
this case, the \textit{next} iteration will return it as an output. We can now
quite easily state the semantics of \rle{where}: we simply precompose the
semantics of the entry block \(\dnt{\haslb{\Gamma}{t}{\ms{L}}}\) with the
\textit{iterated} semantics of the associated control-flow graph,
\(\dnt{\lhaslb{\ms{L}}{L}{\ms{K}}}^\dagger\).
