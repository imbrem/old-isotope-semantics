...
\textit{label-substitution}, which will require introducing some
substitution judgements to state properly, which we do in Figure
\ref{fig:ssa-meta-judgements}. In particular, the judgement
\(\issubst{\gamma}{\Theta}{\Gamma}\) says that \(\gamma\), which is
taken to be a map from variable names to expressions, is a valid
substitution taking \(\Theta\) to \(\Gamma\). What this means is that,
for each variable \(\thyp{x}{A}{q}\) in \(\Gamma\), \(\gamma(x)\) can
be interpreted as a \textit{central} term of type \(A^q\) in a
subcontext of \(\Theta\), such that when this is done for all
\(\thyp{x}{A}{q} \in \Gamma\) any non-affine variables in \(\Theta\)
are used up: this is described formally via the typing rules
\rle{subst-nil}, which says that any function \(\gamma\) takes the
empty context to itself, and \rle{subst-cons}, which says that if
\(\gamma\) takes \(\Theta_\Gamma\) to \(\Gamma\),
\(\hasty{\Theta_x}{1}{\gamma(x)}{A}{q}\),
\(q \subseteq \ms{lin}(\Theta_x)\) (to avoid necessating the
duplication/discarding of substructural variables), and \(\Theta\)
splits into \(\Theta_\Gamma\) and \(\Theta_x\), then
\(\issubst{\gamma}{\Theta}{\Gamma, \thyp{x}{A}{q}}\).
...
particular, we begin by interpreting the judgement that a type is affine,
\(\tlin{A}{\taff}\), as the drop morphism \(\ms{drop}(\dnt{A}): \mc{C}(\dnt{A},
I)\) allowing us to discard the type by sending it to the monoidal unit; type
being relevant, \(\tlin{A}{\trel}\) is similarly interpreted as the split
morphism \(\ms{drop}(\dnt{A}): \mc{C}(\dnt{A}, \dnt{A} \otimes \dnt{A})\). We
can then interpret a weakening \(\csplits{\Gamma}{\Delta}{\Xi}\) as a pure
morphism \(\mc{C}^\tint_1(\dnt{\Gamma}, \dnt{\Delta} \otimes \dnt{\Xi})\) which
partitions the variables as appropriate, splitting and dropping as necessary;
weakenings are then just interpreted as the postcomposition of the associated
split morphism and the unitor.
...
\paragraph{Weak memory} 

\TODO{\citet{leaky-semicolon} for weak memory stuff: future work: add while loops, and also refinement}
\TODO{\citet{paviotti-modular-relaxed-dep-20}}
\TODO{\citet{sparky}}

\todo{Main thing here to say is that we need semantics for while loops, plus associativity of semicolon, if statements must support distributivity}

\paragraph{Type Theory for SSA}

\TODO{\citet{hua-explicit-ssa-2010}}
\TODO{\citet{ssa-types-matsuno-ohori-06}}
\TODO{\citet{typed-effect-ssa-rigon-torrens-vasconcellos-20}}

\paragraph{Functional SSA}

\TODO{\citet{chakravarty-functional-ssa-2003}}

\subparagraph{Operational Semantics}

\TODO{\citet{barthe-compcert-ssa-2014}}
\TODO{\citet{buchwald-verified-ssa-2016}}
\TODO{\citet{herklotz-gsa-2023}}
\TODO{\citet{garbuzov-structural-cfg-2018}}
\TODO{\citet{vellum}}

\subparagraph{Denotational Semantics}

\TODO{\citet{pop-ssa-inout-2009}}
