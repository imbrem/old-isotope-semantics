...
\TODO{go cite quantiatitive type theory}
...
Given a resource algebra \(R\), we define a \textbf{subalgebra} to be given by
operations \(+', \leq'\) such that \(+ \subseteq +\), \(\leq \subseteq \leq'\)
satisfying the resource algebra laws. In particular, every resource algebra has
the minimal subalgebra \(+_\varnothing\) defining only \(a + 0 = 0 + a = a\)
